            ...
            if (!err)
            {
                ...
                else
                {
                    ...
                    for (; syms; syms = syms->next)
                    {
                        ...
                        syms->data = sa_text + syms->dataLength;
                        next = (syms->next) ? syms->next->dataLength : sa_ntext;
                        assert(next > syms->dataLength);
                        syms->dataLength = next - syms->dataLength - 1;
                    }
