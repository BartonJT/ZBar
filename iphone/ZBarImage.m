...
{
    unsigned long format = zbar_image_get_format(zimg);
...
- (unsigned) sequence
{
    unsigned sequence = zbar_image_get_sequence(zimg);
...
- (CGRect) crop
{
    ...
    CGRect cropFrame = CGRectMake(x, y, w, h);
    return cropFrame;
}
...
{
    ZBarSymbolSet *symbols = [[[ZBarSymbolSet alloc]
                               initWithSymbolSet:zbar_image_get_symbols(zimg)] autorelease];
    return symbols;
}
...
{
    unsigned long dataLength = zbar_image_get_data_length(zimg);
    return dataLength;
}
...
- (UIImage *) UIImage
{
    UIImage *anImage = [self UIImageWithOrientation:UIImageOrientationUp];
    return anImage;
}
